...
true
  ...
  - [Holiday Modification](#holiday-modification)
...
parameters. There are 5 parameters
\(\{\alpha, \beta, \gamma, \omega, \phi \}\) in the model. We use grid
search to find a set of optimal parameters. The searching process has
been optimized by parallel computing.
...
```
    ...
    ## grid search: 119.19 sec elapsed

The list `select.ls` contains the model which has the smallest MAPE for
validation set, and the validation resutls.
...
We pick the parameters which yeild the lowest
MAPE.
...
![Prediction of FY20](README_files/figure-gfm/plot-1.png)
